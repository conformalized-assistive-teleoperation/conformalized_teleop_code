...
from utils.remapping_dataset import RemappedDataset
from utils.robot_7dof_goal_utils.train_teleop_model import train_telop_controller_with_quantiles, train_ensemble_teleop_controller
...
if __name__ == '__main__':
    ...
    if calibration_type == 'acqr':
        from utils.robot_7dof_goal_utils.calibrate_teleop_model import calibrate_teleop_model_with_acqr as calibrate_teleop_model
    elif calibration_type == 'qr':
        from utils.robot_7dof_goal_utils.calibrate_teleop_model import calibrate_teleop_model_with_qr as calibrate_teleop_model
    elif calibration_type == 'ensemble':
        from utils.robot_7dof_goal_utils.calibrate_teleop_model import calibrate_teleop_model_with_ensemble as calibrate_teleop_model
    else:
    ...
    with open(indistr_alice_file, 'rb') as f:
        ...
        # select [['006', '007']] from indistr_alice_data
        if uncertainty_context == 'latent_prefs':
            indistr_alice_data = {key: indistr_alice_data[key] for key in ['006', '007']}
        elif uncertainty_context == 'control_prec':
            indistr_alice_data = {key: indistr_alice_data[key] for key in ['006']}
    ...
    if uncertainty_context == 'latent_prefs':
        ...
        with open(indistr_bob_file, 'rb') as f:
            ...
            if uncertainty_context == 'latent_prefs':
                indistr_bob_data = {key: indistr_bob_data[key] for key in ['006', '007']}
            elif uncertainty_context == 'control_prec':
                indistr_bob_data = {key: indistr_bob_data[key] for key in ['006']}
        if calibration_type == 'acqr':
