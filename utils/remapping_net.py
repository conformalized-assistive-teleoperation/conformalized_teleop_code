...
class FC_Action_Remapper_with_Quantiles(torch.nn.Module):
    def __init__(
        self,
        ...
        super(FC_Action_Remapper_with_Quantiles, self).__init__()
    ...
    def build_model(self) -> None:
        # Encoder --> Takes (State, Action) --> Encodes to `z` latent space, then Decode to Action plus Quantiles
        self.enc = nn.Sequential(
            nn.Linear(self.state_dim + self.latent_dim, self.hidden_dim),
            nn.GELU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.GELU(),
            nn.Linear(self.hidden_dim, self.latent_dim * 3),
            nn.Tanh(),
            nn.Linear(self.latent_dim * 3, self.hidden_dim),
            nn.GELU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.GELU(),
            nn.Linear(self.hidden_dim, self.action_dim * 3),
        )
